...
def basic_test_n_step():
    ...
    td_buffer = ReplayBuffer(storage=ListStorage(max_size=100), collate_fn=lambda x: x, batch_size=5)
    ...
    for i in range(1, 6):
        ...
        t.truncated = i >= 20 and i % 5 == 0
        t.action = torch.ones(1)

        nt = Timestep(i + 1)
        nt.obs = torch.zeros(3, 4)

        if t.truncated:
            truncated = Timestep(113 * (i + 1))
            truncated.obs = torch.ones(3, 4) * -i
    ...
    t = timesteps[0]
    condition = not t.needs_n_step and t.n_step_return - 5.9203 < 1e-5 and t.n_step_gamma - 0.9703 < 1e-5 and t.n_step_done is False
    assert condition, "TIMESTEP {} FAILED\n{}".format(t.id, t)

    t = t.next()
    condition = not t.needs_n_step and t.n_step_return - 8.8904 < 1e-5 and t.n_step_gamma - 0.9703 < 1e-5 and t.n_step_done is False
    assert condition, "TIMESTEP {} FAILED\n{}".format(t.id, t)

    t = t.next()
    condition = not t.needs_n_step and t.n_step_return - 11.8605 < 1e-5 and t.n_step_gamma - 0.9703 < 1e-5 and t.n_step_done is True
    assert condition, "TIMESTEP {} FAILED\n{}".format(t.id, t)

    t = t.next()
    condition = not t.needs_n_step and t.n_step_return - 8.95 < 1e-5 and t.n_step_gamma - 0.9801 < 1e-5 and t.n_step_done is True
    assert condition, "TIMESTEP {} FAILED\n{}".format(t.id, t)

    t = t.next()
    condition = not t.needs_n_step and t.n_step_return - 5 < 1e-5 and t.n_step_gamma - 0.99 < 1e-5 and t.n_step_done is True
    assert condition, "TIMESTEP {} FAILED\n{}".format(t.id, t)

    print("BASIC TEST PASSED")


def test_partial_n_step():
...
def test_save_load():
    ...
    td_buffer = PrioritizedReplayBuffer(storage=ListStorage(max_size=100000), collate_fn=lambda x: x, batch_size=5,
                                        alpha=0.5, beta=0.5)
    buffer = TimestepBuffer(td_buffer, frame_stack=1, device="cpu", n_step=3, gamma=0.99)
    t = Timestep(1)
    t.obs = torch.zeros(3, 4)
    print("filling buffer")
    for i in range(1, 100000):  # (1, 27)
        t.reward = i
        t.done = 20 > i > 0 and i % 5 == 0
        t.truncated = i >= 10 and i % 12 == 0
        t.action = torch.ones(1)
